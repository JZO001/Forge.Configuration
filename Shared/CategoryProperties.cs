...
namespace Forge.Configuration.Shared
{
    ...
    public class CategoryPropertyItems : ConfigurationElementCollection, ICloneable, IEnumerable<CategoryPropertyItem>, IPropertyItem
    {
        ...
        public CategoryPropertyItem this[int index]
        {
            get
            {
                CategoryPropertyItem item = BaseGet(index) as CategoryPropertyItem;
                if (item != null)
                {
                    item.Parent = this;
                }
        ...
        public new CategoryPropertyItem this[string key]
        {
            get
            {
                CategoryPropertyItem item = BaseGet(key) as CategoryPropertyItem;
                if (item != null)
                {
                    item.Parent = this;
                }
        ...
        public ICollection<CategoryPropertyItem> GetCategoryItems(string categoryName)
        {
            ...
            if (Count > 0)
            {
                ...
                {
                    CategoryPropertyItem item = (CategoryPropertyItem)BaseGet(i);
        ...
        /// <summary>Gets the property items.</summary>
        /// <value>The property items.</value>
        [DebuggerHidden]
        public Dictionary<string, IPropertyItem> Items
        {
            get
            {
                Dictionary<string, IPropertyItem> result = new Dictionary<string, IPropertyItem>();
                IEnumerator<CategoryPropertyItem> piEnum = GetEnumerator();
                while (piEnum.MoveNext())
                {
                    result.Add(piEnum.Current.Id, piEnum.Current);
                }
                return result;
            }
        }
        ...
        public object Clone()
        {
            ...
            if (Count > 0)
            {
                for (int i = 0; i < Count; i++)
                {
                    items.Add(((ICloneable)BaseGet(i)).Clone() as CategoryPropertyItem);
                }
            }
            return items;
        }
    ...
    /// </summary>
    [Serializable]
    public class CategoryPropertyItem : ConfigurationElement, ICloneable, IEnumerable<CategoryPropertyItem>, IPropertyItem
    {
        ...
        /// <value>The value.</value>
        [DebuggerHidden]
        public string Value { get => EntryValue; set => EntryValue = value; }
        ...
        public Dictionary<string, IPropertyItem> Items
        {
            ...
            {
                Dictionary<string, IPropertyItem> result = new Dictionary<string, IPropertyItem>();
                IEnumerator<CategoryPropertyItem> piEnum = PropertyItems.GetEnumerator();
                while (piEnum.MoveNext())
                {
                    result.Add(piEnum.Current.Id, piEnum.Current);
                }
        ...
        public object Clone()
        {
            ...
            item.PropertyItems = PropertyItems.Clone() as CategoryPropertyItems;
            return item;
        }
        ...
        {
            PropertyItem result = new PropertyItem(Id, EntryValue);

            foreach (CategoryPropertyItem pi in this.PropertyItems)
            {
                result.Items[pi.Id] = pi.CreatePropertyItem();
            }
